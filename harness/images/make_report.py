...
AI_LAB = "[AI-Lab](https://github.com/marianvid/ai-lab)"
...
def main() -> int:
    ...
    lines = [
        ...
        "",
        f"These results measure the deployed **{AI_LAB} system**, including its gateway and engine adapter. They are not bare GPU kernel timings.",
        "",
        f"| | Linux {AI_LAB} | macOS {AI_LAB} |",
        "|---|---|---|",
        ...
        f"| Model storage | Two internal Lexar NM790 4 TB NVMe drives | Local/external {AI_LAB} model roots configured for the benchmark |",
        f"| Operating environment | Proxmox VE host; engines run inside an **unprivileged LXC container** | macOS 15.7.3; {AI_LAB} runs natively, without LXC |",
        f"| {AI_LAB} role | Gateway, model loading/unloading, request routing and timing | Native manager using an isolated benchmark configuration on `127.0.0.1:8110` |",
        f"| Engine supervision | systemd, one unit per configured model instance | {AI_LAB} starts and supervises the local engine processes |",
        f"| Image runtime | ComfyUI behind the private {AI_LAB} adapter; NVIDIA driver 610.57.04 and CUDA 13 | ComfyUI 0.34.0 behind the same adapter; Python 3.11.7, torch 2.13.0 and Apple MPS |",
        f"| OCR runtime | PaddleOCR 3.x through {AI_LAB}'s isolated OCR service | PaddleOCR 3.x in an isolated local runtime managed by {AI_LAB} |",
        ...
        "",
        f"The OCuLink connection affects model loading because weights cross the cable, but not image generation after the model is resident in GPU memory. Request timing includes the {AI_LAB} HTTP path and engine adapter because that is the system a real client uses.",
        "",
        "## Model settings",
        "",
